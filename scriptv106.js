...
document.getElementById('champion-form').addEventListener('submit', function(event) {
    ...
    const resultText = `Doğum yılınızdan itibaren ${selectedTeam} ${teamChampionships} kez şampiyon olmuştur. 
    Bu da ${totalYears} yıllık süreçte ${(ratio * 100).toFixed(2)}% oranına denk gelmektedir.`;
    document.getElementById('result').innerText = resultText;

    const championshipsDiv = document.getElementById('championships');
    championshipsDiv.innerHTML = ''; // Önceki sonuçları temizleyin

    filteredData.forEach(data => {
        if (data.champion === selectedTeam) {
            ...
            cupImage.src = 'https://www.tff.org/Resources/TFF/FileGallery/3064b5bd0b214af49c4842dd714f7674_m.JPG'; // Kupa görsel URL'si
            cupImage.alt = 'Süper Lig';

            const seasonText = document.createElement('div');
            seasonText.innerText = `${data.year - 1}-${data.year}`; // Sezon formatında gösterim

            championshipDiv.appendChild(cupImage);
            championshipDiv.appendChild(seasonText);
            championshipsDiv.appendChild(championshipDiv);
        }
    });
});
